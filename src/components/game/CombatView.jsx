...
function Bar({ value, max, color, label }) {
  ...
  return (
    <div>
      <div className="text-[10px] text-stone-400 mb-0.5">{label} {value}/{max}</div>
        ...
        <div className="max-w-md w-full rounded-2xl border border-emerald-800/50 bg-[#0d1410] p-6 text-center animate-pop">
          ...
          {c.result === 'victory' && c.rewards && (
            <div className="text-xs text-amber-200 mb-3">
              ...
              <div>+{c.rewards.spiritStones} stones · +{c.rewards.exp} insight</div>
            </div>
          )}
      ...
      <div className="max-w-lg w-full rounded-2xl border border-rose-900/50 bg-[#0d1410] p-4 animate-pop">
        <div className="grid grid-cols-2 gap-3 mb-3">
          <div className="rounded-lg border border-emerald-800/40 bg-emerald-900/10 p-3">
            <div className="flex items-center gap-2 mb-2"><span className="text-2xl">🧑‍🌾</span><div><div className="text-sm font-semibold text-emerald-100">{p.name}</div><div className="text-[10px] text-stone-400">{REALMS[p.realmIndex].name}</div></div></div>
          ...
          <div className="rounded-lg border border-rose-800/40 bg-rose-900/10 p-3">
            ...
            {c.revealed && <div className="text-[10px] text-amber-300/80 mt-1">Weakness: {enemy.weakness}</div>}
        ...
        <div className="text-[10px] text-stone-500 mb-1">Use Gu:</div>
        <div className="grid grid-cols-2 gap-1.5 mb-2">
          {equipped.map(inst => {
            const gu = GU_BY_ID[inst.guId];
            const cd = c.cooldowns[inst.instanceId] || 0;
            const noEssence = p.primevalEssence < gu.energyCost;
            ...
            return (
              <button key={inst.instanceId} disabled={disabled}
                ...
                <div className="text-[9px] text-stone-400">{gu.type} · ⚡{gu.energyCost}{cd > 0 ? ` · ⏳${cd}` : ''}</div>
              </button>
            );
          })}
        </div>
