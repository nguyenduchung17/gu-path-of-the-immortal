...
const TABS = [
  ...
  { id: 'dao', label: 'Dao Mastery', icon: '☯️' },
      ...
      <div className="max-w-6xl mx-auto px-3 py-2">
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <div className="flex-1 min-w-[140px] grid grid-cols-2 gap-2 max-w-xs">
            <div>
              <div className="text-[10px] text-rose-300/80">HP {Math.floor(p.hp)}/{p.maxHp}</div>
              <Bar value={p.hp} max={p.maxHp} color="linear-gradient(90deg,#e11d48,#fb7185)" />
            </div>
            <div>
              <div className="text-[10px] text-sky-300/80">Essence {Math.floor(p.primevalEssence)}/{p.maxPrimevalEssence}</div>
