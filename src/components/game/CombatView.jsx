...
export default function CombatView() {
  ...
  const [castColor, setCastColor] = useState(null);
  ...
  useEffect(() => {
    ...
    let enemyDmg = 0, playerDmg = 0, dodge = false;
    for (const l of lines) {
      if (/strikes .*? for (\d+) damage/.test(l)) enemyDmg += Number(l.match(/for (\d+) damage/)[1]);
      else if (/is cut by thorns for (\d+) damage/.test(l)) enemyDmg += Number(l.match(/for (\d+) damage/)[1]);
      else if (/attacks for (\d+) damage/.test(l)) playerDmg += Number(l.match(/for (\d+) damage/)[1]);
      else if (l.includes('dodge')) dodge = true;
    }
    setFx({ key: logLen, enemyDmg, playerDmg, dodge, castColor, projectile: !!castColor });
    if (enemyDmg > 0) sfx('hit');
  ...
  if (c.over) {
    return (
      <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-[2px] flex items-center justify-center p-4">
        <div className="max-w-md w-full rounded-xl border-2 border-emerald-800/60 bg-[#0d1410] p-6 text-center animate-pop">
          <div className="text-4xl mb-2">{c.result === 'victory' ? '⚔️' : c.result === 'flee' ? '🌫️' : '💀'}</div>
          <h2 className="text-xl font-heading font-semibold text-emerald-100 mb-2">
  ...
  const equipped = state.player.equippedGu.map(id => state.player.ownedGu.find(g => g.instanceId === id)).filter(Boolean);
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/85 backdrop-blur-[2px] flex items-center justify-center p-3">
      <div className="max-w-lg w-full rounded-xl border-2 border-rose-900/60 bg-[#0d1410] p-4 animate-pop">
        {/* battle stage */}
        <div className="mb-3">
          <BattleScene enemy={enemy} fx={fx} />
        </div>

        <div className="grid grid-cols-2 gap-3 mb-3">
          <div className="rounded-lg border border-emerald-800/40 bg-emerald-900/10 p-3">
            <div className="flex items-center justify-between mb-2">
              <div className="text-sm font-semibold text-emerald-100">{p.name}</div>
              <div className="text-[10px] text-stone-400">{stage.name}</div>
            </div>
          ...
          <div className="rounded-lg border border-rose-800/40 bg-rose-900/10 p-3">
            <div className="flex items-center justify-between mb-2">
              <div className="text-sm font-semibold text-rose-100">{enemy.name}</div>
              <div className="text-[10px] text-stone-400">ATK {enemy.attack} · DEF {enemy.defense}</div>
            </div>
        ...
        <div className="flex gap-1.5 flex-wrap">
          ...
          <button onClick={() => { sfx('ui'); dispatch({ type: 'PLAYER_ACTION', action: 'flee' }); }}
